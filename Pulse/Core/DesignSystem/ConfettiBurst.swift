...
/// 36 small rectangles bursting from centre, drifting downward.
/// Pure SwiftUI, no external deps. Respects Reduce Motion.
struct ConfettiBurst: View {
    @State private var animate = false
    @Environment(\.accessibilityReduceMotion) private var reduceMotion

    private let particles: [Particle] = (0..<36).map { _ in Particle.random() }
    ...
    private struct Particle {
        ...
        static func random() -> Particle {
            ...
            let radius = Double.random(in: 130...280)
            let colors: [Color] = [
            ...
            return Particle(
                ...
                endY: CGFloat(sin(angle) * radius) + 220,
